    ...
    return (
          <div className="bg-white rounded-lg p-4 border border-violet-100 shadow-sm">
                <div className="flex items-center justify-between mb-1">
                        <span className="text-slate-500 text-xs uppercase tracking-wide">{label}</span>span>
...
function FilterBadge({ filter, negate }: { filter: SegmentFilter; negate?: boolean }) {
    return (
          <span
                ...
                >
            {negate && <span className="font-bold text-rose-500">NOT</span>span>}
...
export default function CampaignDetails({ campaign }: Props) {
    ...
    useEffect(() => {
          ...
          // Fetch MoEngage campaign performance stats with click-through attribution
          fetch(
                  `/api/campaign-stats?campaignId=${encodeURIComponent(campaign.id)}&channel=${encodeURIComponent(
    ...
    const isAllUsers = ep.targetAudience === 'All Users';
    const includedFilters: SegmentFilter[] = ep.includedFilters || [];
    const excludedFilters: SegmentFilter[] = ep.excludedFilters || [];
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-violet-100">
            {/* Header */}
                <div className="flex items-start justify-between mb-5">
                        <div>
                                  <h2 className="text-xl font-bold text-violet-900 mb-1">{campaign.title}</h2>h2>
            ...
            {/* Target Users */}
                <div className="mb-4">
                        <h3 className="text-xs font-semibold text-violet-400 uppercase tracking-wide mb-2">Target Users</h3>h3>
                        <div className="bg-violet-50 rounded-lg p-3 border border-violet-100">
                                  <div className="flex items-center gap-2 mb-1">
                                              <span className="text-lg">{isAllUsers ? '🌐' : '🎯'}</span>span>
                                              <span className="text-violet-800 font-semibold text-sm">
                                                {ep.targetAudience || (isAllUsers ? 'All Users' : 'Segmented Users')}
                                              </span>span>
                                  </div>div>
            ...
            {/* Performance Metrics */}
                ...
                <p className="text-xs text-violet-300 mb-3 flex items-center gap-1">
                        <span>📊</span>span>
                        Campaign stats · Attribution: <span className="font-semibold text-violet-400">Click-through</span>span>
                </p>p>
            ...
            {loading ? (
                    ...
                    <div className="bg-rose-50 border border-rose-200 rounded-lg p-3 text-rose-600 text-sm">
                              <span className="font-medium">Could not load stats:</span>span> {statsError}
                    </div>div>
                  ) : metrics ? (
                    ...
                    <p className="text-violet-300 text-sm text-center py-4">No metrics available</p>p>
                )}
          </div>div>
        );
}
