...
const GROUP_COLORS: Record<string, string> = {
  ...
  CONNECTOR:      'bg-slate-100 text-slate-600 border-slate-200',
};
...
/** Normalise a raw campaignType string to the human-readable DeliveryType label. */
function resolveDeliveryTypeLabel(raw: string | undefined): string | null {
  if (!raw) return null;
  // Already a human label (e.g. 'One Time')
  if (Object.values(API_DELIVERY_TYPE_MAP).includes(raw as any)) return raw;
  // Try the API_DELIVERY_TYPE_MAP (e.g. 'ONE_TIME' → 'One Time')
...
export default function CampaignDetails({ campaign }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-violet-100">
      ...
      {/* ── Header ──────────────────────────────────────────────────────── */}
      <div className="mb-5">
        <h2 className="text-xl font-bold text-violet-900 mb-2">{campaign.title}</h2>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Delivery type badge — only if this channel has sub-types */}
          {showDeliveryType && (
          ...
          {/* Direct-launch indicator — only for channels with no sub-types */}
          {channelDirect && (
      ...
      {/* ── Performance Metrics — only when real data is available ────────── */}
      {loading && (
